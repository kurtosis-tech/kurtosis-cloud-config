...
apt-get "$APT_LOCK_TIMEOUT" update
apt-get "$APT_LOCK_TIMEOUT" install -y ca-certificates curl gnupg lsb-release jq docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin
...
apt-get "$APT_LOCK_TIMEOUT" update
apt-get "$APT_LOCK_TIMEOUT" install -y kurtosis-cli
